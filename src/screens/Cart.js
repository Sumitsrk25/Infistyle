      ...
      Alert.alert("Warning", "Are you sure you want to delete from Cart ?", [
        {
          ...
          onPress: async () => {
            ...
            if (data.status == "success") {
              ...
              //   (item) => item.iChallengePicId !== iChallengePicId
              // );
              // setMyDatapic(updatedData);
              // getUserDatapic()
              //   .then(setMyDatapic)
              //   .finally(() => setIsRefreshing(false)); // await means till it dowsnt get data it will not execute function below it

              // // Update extraData to trigger a re-render
              // setExtraData(new Date().getTime()); // or any unique value
